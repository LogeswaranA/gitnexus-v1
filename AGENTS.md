...
| Date | Version | Change |
|------|---------|--------|
| 2026-06-16 | 1.8.0 | Added `git_log` and `git_show` MCP tools for commit history browsing; REST endpoints `GET /api/git-log` and `GET /api/git-show`; web `GitHistoryPanel` component. |
...
This project is indexed by GitNexus as **GitNexus** (28738 symbols, 45716 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
Two MCP tools expose the commit history captured during `gitnexus analyze`:

| Tool | Purpose |
|------|---------|
| `git_log` | Paginated commit history. Filters: `author` (substring), `search` (subject keyword), `since`/`until` (ISO 8601). Use `summary_only: true` for branch name, total commits, and top contributors without loading the full list. Default limit 50, max 500. |
| `git_show` | Full detail for one commit: files changed with status (added/modified/deleted/renamed), per-file insertion/deletion counts, and (optionally) the full patch diff. Pass `include_diff: true` for the raw patch. |

**Data flow:** `gitnexus analyze` captures history → `.gitnexus/git-log.json`; `git_log` reads from that file. `git_show` shells out to `git show` / `git diff-tree` live against the indexed repo path, so it always reflects the actual on-disk state.

**Next-step hints:** after `git_log`, use `git_show({commit: "<hash>"})` to inspect a specific commit. After `git_show`, use `query({query: "<filename or symbol>"})` to explore the changed code in the knowledge graph.

**HTTP equivalents** (for the web UI and direct API clients):
- `GET /api/git-log?repo=<name>&limit=&offset=&author=&since=&until=&search=&summary_only=`
- `GET /api/git-show?commit=<hash>&repo=<name>&include_diff=`
...
Commit history is surfaced at three layers:

| Layer | Location | Description |
|-------|----------|-------------|
| **MCP tools** | `gitnexus/src/mcp/tools.ts` (`git_log`, `git_show`) | Tool definitions exposed to AI agents via MCP protocol. |
| **Backend impl** | `gitnexus/src/mcp/local/local-backend.ts` (`gitLog`, `gitShow`) | `gitLog` reads `.gitnexus/git-log.json` (written by `gitnexus analyze`). `gitShow` calls `getCommitShow()` in `storage/git.ts` which shells to `git show` + `git diff-tree`. |
| **HTTP API** | `gitnexus/src/server/api.ts` | `GET /api/git-log` and `GET /api/git-show` — proxy query params to the corresponding `callTool` invocations. |
| **Web UI** | `gitnexus-web/src/components/GitHistoryPanel.tsx` | Paginated commit list with search/filter controls (message, author, since date). Click a commit to expand it and see per-file stats (insertions/deletions, status color-coded green/amber/red). |
| **Client types** | `gitnexus-web/src/services/backend-client.ts` | `GitCommitInfo`, `GitLogSummary`, `GitLogResult`, `GitShowFile`, `GitShowResult`; `fetchGitLog()`, `fetchGitShow()`. |

`git_log` data is captured at index time (`gitnexus analyze`); it reflects the history as of the last analysis run.
`git_show` data is live (shells out on demand), so it can inspect any commit reachable from the indexed repo path even if it post-dates the analysis.
