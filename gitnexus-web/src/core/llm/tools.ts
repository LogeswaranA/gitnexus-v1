...
/**
 * Backend query interface for Git history tools.
 */
export interface GitHistoryBackend {
  gitLog: (opts?: {
    repo?: string;
    limit?: number;
    offset?: number;
  ...
  gitShow: (commit: string, opts?: { repo?: string; include_diff?: boolean }) => Promise<unknown>;
}

/**
 * Tool factory - creates tools bound to backend HTTP query functions
 */
export const createGraphRAGTools = (backend: GraphRAGBackend, gitBackend?: GitHistoryBackend) => {
...
- Risk summary (based on direct callers, processes, clusters)`,
  ...
  const gitLogTool = tool(
    async ({
      ...
      if (!gitBackend) {
        return 'Git history is not available in this configuration.';
      }
      try {
        const result = (await gitBackend.gitLog({
          summary_only,
          author,
          since,
          until,
        ...
        if (result?.error) {
          return `Git log error: ${result.error}`;
        }
        ...
        if (result.summary) {
          ...
          lines.push(`BRANCH: ${s.currentBranch ?? 'unknown'}`);
          lines.push(`TOTAL COMMITS: ${s.totalCommits ?? 0}`);
          if (s.latestCommit) {
            lines.push(
              `LATEST: ${s.latestCommit.shortHash ?? s.latestCommit.hash?.slice(0, 7)} — ${s.latestCommit.subject} (${s.latestCommit.author}, ${s.latestCommit.date?.slice(0, 10)})`,
            );
          }
          if (s.contributors?.length > 0) {
            lines.push(
              `TOP CONTRIBUTORS: ${s.contributors
                .slice(0, 5)
                .map((c: any) => `${c.name} (${c.commits})`)
                .join(', ')}`,
            );
          }
          if (summary_only) return lines.join('\n');
          lines.push('');
        }

        const commits: any[] = result.commits ?? [];
        if (commits.length === 0) {
          lines.push('No commits found matching the filters.');
          return lines.join('\n');
        }

        lines.push(
          `COMMITS (${result.offset ?? 0}–${(result.offset ?? 0) + commits.length} of ${result.total ?? commits.length}):`,
        );
        for (const c of commits) {
          const hash = c.shortHash ?? c.hash?.slice(0, 7) ?? '?';
          const date = c.date?.slice(0, 10) ?? '';
          lines.push(`${hash}  ${date}  ${c.author}  ${c.subject}`);
        }
        if (result.has_more) {
          lines.push(
            `\n(${result.total - (result.offset ?? 0) - commits.length} more — use offset to paginate)`,
          );
        }
        return lines.join('\n');
      } catch (error) {
        return `Git log error: ${error instanceof Error ? error.message : String(error)}`;
      }
    },
    {
      name: 'git_log',
      description:
        'Access the git commit history. Returns commits with hash, author, date, and subject. Use summary_only for branch/contributor stats. Filter by author, date range, or keyword.',
      schema: z.object({
        summary_only: z
          .boolean()
          .optional()
          .nullable()
          .describe('Return only branch/contributor summary (default: false)'),
        author: z.string().optional().nullable().describe('Filter by author name or email substring'),
        since: z
          .string()
          .optional()
          .nullable()
          .describe('Include commits on or after date (ISO 8601, e.g. "2024-01-01")'),
        until: z
          .string()
          .optional()
          .nullable()
          .describe('Include commits on or before date (ISO 8601, e.g. "2024-12-31")'),
        search: z.string().optional().nullable().describe('Filter commits by subject line keyword'),
        limit: z
          .number()
          .optional()
          .nullable()
          .describe('Max commits to return (default: 50, max: 500)'),
        offset: z.number().optional().nullable().describe('Pagination offset (default: 0)'),
      }),
    },
  );

  // ============================================================================
  // TOOL 9: GIT SHOW (Commit detail — files changed + optional diff)
  // ============================================================================

  const gitShowTool = tool(
    async ({ commit, include_diff }: { commit: string; include_diff?: boolean }) => {
      if (!gitBackend) {
        return 'Git history is not available in this configuration.';
      }
      try {
        const result = (await gitBackend.gitShow(commit, { include_diff })) as any;

        if (result?.error) {
          return `git show error: ${result.error}`;
        }

        const c = result.commit;
        const lines: string[] = [
          `COMMIT: ${c.hash ?? commit}`,
          ...
          ...(c.body ? [``, `    ${c.body}`] : []),
          ``,
          `STATS: ${result.stats?.filesChanged ?? 0} files changed, +${result.stats?.insertions ?? 0} insertions, -${result.stats?.deletions ?? 0} deletions`,
          ``,
          `FILES CHANGED:`,
        ];
        ...
        for (const f of result.files ?? []) {
          ...
          const delta =
            f.insertions || f.deletions ? ` (+${f.insertions ?? 0} -${f.deletions ?? 0})` : '';
          lines.push(`  ${status} ${f.path}${delta}`);
        }

        if (include_diff && result.diff) {
          lines.push(``, `DIFF:`, result.diff);
        } else if (!include_diff) {
          lines.push(``, `(Re-run with include_diff: true to see the full patch)`);
        }

        return lines.join('\n');
      } catch (error) {
        return `git show error: ${error instanceof Error ? error.message : String(error)}`;
      }
    },
    {
      name: 'git_show',
      description:
        'Show full details of a specific git commit: files changed with insertions/deletions, and optionally the full diff. Use after git_log returns a commit hash to inspect.',
      schema: z.object({
        commit: z.string().describe('Commit hash to inspect (full or short, e.g. "e0aaa2c")'),
        include_diff: z
          .boolean()
          .optional()
          .nullable()
          .describe('Include the full line-by-line diff patch (default: false)'),
      }),
    },
  );
  ...
  const tools: any[] = [searchTool, cypherTool, grepTool, readTool, overviewTool, exploreTool, impactTool];
  if (gitBackend) {
