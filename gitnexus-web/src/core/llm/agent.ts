...
import { createGraphRAGTools, type GraphRAGBackend, type GitHistoryBackend } from './tools';
import type {
  ProviderConfig,
  OpenAIConfig,
  AzureOpenAIConfig,
...
- **\`git_log\`** — Git commit history. Filter by author, date range, keyword. Use summary_only for branch/contributor stats.
- **\`git_show\`** — Full detail for a single commit: files changed with +/- line counts. Set include_diff: true for the full patch. Always call this after git_log when the user asks "what changed in commit X" or "explain the changes".
...
export const createGraphRAGAgent = (
  ...
  gitBackend?: GitHistoryBackend,
) => {
  const model = createChatModel(config);
  const tools = createGraphRAGTools(backend, gitBackend);
