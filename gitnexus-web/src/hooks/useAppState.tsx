  ...
  const initializeAgent = useCallback(
    async (overrideProjectName?: string): Promise<void> => {
      ...
      try {
        ...
        const gitBackend = {
          gitLog: (opts?: any) => fetchGitLog(repo, opts),
          gitShow: (commit: string, opts?: any) => fetchGitShow(commit, repo, opts?.include_diff),
        };

        agentRef.current = createGraphRAGAgent(config, backend, codebaseContext, gitBackend);
