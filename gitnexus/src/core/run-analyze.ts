...
export interface AnalyzeResult {
  ...
  /** Summary of the repo's git commit history captured during this analysis run. */
...
export async function runFullAnalysis(
  ...
  try {
    ...
    // ── Phase 4.5: Capture git commit history (best-effort) ──────────
    let gitLog: GitLogSummary | undefined;
    if (repoHasGit) {
      try {
        progress('done', 96, 'Capturing git history...');
        const commits = getCommitHistory(repoPath);
        if (commits.length > 0) {
          gitLog = buildGitLogSummary(repoPath, commits);
          await saveGitLog(storagePath, commits);
          log(`Git history: ${commits.length} commits captured`);
        }
      } catch {
        // Never fail the analysis for git history capture
      }
    }
