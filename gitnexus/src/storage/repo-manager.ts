...
export interface RepoMeta {
  ...
  /** Summary of the repo's git commit history captured at index time. */
  gitLog?: GitLogSummary;
}
...
const GIT_LOG_FILE = 'git-log.json';
...
/**
 * Persist the full git commit history to .gitnexus/git-log.json.
 * Uses atomic tmp-file + rename to prevent partial writes.
 */
export const saveGitLog = async (storagePath: string, commits: CommitInfo[]): Promise<void> => {
  await fs.mkdir(storagePath, { recursive: true });
  const logPath = path.join(storagePath, GIT_LOG_FILE);
  const tmpPath = `${logPath}.tmp`;
  await fs.writeFile(tmpPath, JSON.stringify(commits), 'utf-8');
  await fs.rename(tmpPath, logPath);
};

/**
 * Load the previously captured git commit history from .gitnexus/git-log.json.
 * Returns null when the file does not exist or is unreadable.
 */
export const loadGitLog = async (storagePath: string): Promise<CommitInfo[] | null> => {
  try {
    const raw = await fs.readFile(path.join(storagePath, GIT_LOG_FILE), 'utf-8');
    return JSON.parse(raw) as CommitInfo[];
  } catch {
    return null;
  }
};
