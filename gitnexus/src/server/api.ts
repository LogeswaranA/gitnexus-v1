...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  // Git history — paginated commit log
  app.get('/api/git-log', async (req, res) => {
    try {
      const params: Record<string, any> = { repo: requestedRepo(req) };
      const { limit, offset, author, since, until, search, summary_only } = req.query;
      ...
      if (summary_only === 'true') params.summary_only = true;
      const result = await backend.callTool('git_log', params);
      if (result?.error) {
        res.status(404).json({ error: result.error });
        return;
      }
      ...
      res.status(statusFromError(err)).json({ error: err.message || 'Failed to fetch git log' });
    }
  });
  ...
  app.get('/api/git-show', async (req, res) => {
    try {
      ...
      if (!commit) {
        res.status(400).json({ error: 'Missing "commit" query parameter' });
        return;
      }
      const params: Record<string, any> = {
        repo: requestedRepo(req),
        commit,
        include_diff: req.query.include_diff === 'true',
      };
      const result = await backend.callTool('git_show', params);
      if (result?.error) {
        res.status(404).json({ error: result.error });
        return;
      }
      ...
      res.status(statusFromError(err)).json({ error: err.message || 'Failed to fetch commit' });
    }
  });
