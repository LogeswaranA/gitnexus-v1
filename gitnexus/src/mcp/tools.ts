...
WHEN TO USE: Discover groups before group_sync. Optional "name" returns a single group's config.`,
  ...
  {
    name: 'git_show',
    description: `Show the full details of a single git commit: files changed, insertions/deletions, and optionally the full diff patch.

WHEN TO USE:
- "What changes were made in commit abc1234?"
- "Which files were touched in the last commit?"
- "Show me the diff for this commit"
- Follow-up after git_log returns a commit hash you want to inspect

Returns:
- commit: hash, author, date, subject, full message body
- stats: total files changed, insertions, deletions
- files: per-file list with status (added/modified/deleted/renamed) and line counts
- diff: full patch text (only when include_diff: true)

AFTER THIS: Use query({query: "<filename or concept>"}) to explore the code context around changed files.`,
    annotations: READ_ONLY_TOOL_ANNOTATIONS,
    inputSchema: {
      type: 'object',
      properties: {
        commit: {
          type: 'string',
          description: 'Commit hash to inspect (full or short, e.g. "abc1234" or "a1b2c3d4e5f6...")',
        },
        repo: {
          type: 'string',
          description: 'Repository name or path. Omit if only one repo is indexed.',
        ...
        include_diff: {
          type: 'boolean',
          description:
            'Include the full patch diff text (default: false). Can be large — omit unless you need the exact line changes.',
          default: false,
        },
      },
      required: ['commit'],
  ...
  {
    name: 'git_log',
    description: `Access the full git commit history captured during \`gitnexus analyze\`.

Returns paginated commits with hash, author, date, and subject line.
Also returns a summary: total commits, current branch, top contributors, first/latest commit.

WHEN TO USE:
- "What is my latest commit?" → summary_only: true (latestCommit in response)
- "Who made the last change?" → summary_only: true (latestCommit.author)
- "Who wrote this area of code?" → filter by author
- "What changed last week?" → filter by since/until date
- "Find commits mentioning bug fix" → filter by search keyword
- "How active is this repo?" → use summary_only for contributor stats

FILTERS (all optional, combinable):
- author: substring match on author name or email
- search: substring match on commit subject line
- since / until: ISO 8601 date strings (e.g. "2024-01-01")

PAGINATION: use limit (default 50, max 500) and offset for large histories.
Set summary_only: true to return only the contributor/branch summary without loading commits.

AFTER THIS: Use query() to explore the code changed in commits you find here.`,
    annotations: READ_ONLY_TOOL_ANNOTATIONS,
    inputSchema: {
      type: 'object',
      properties: {
        ...
        since: {
          type: 'string',
          description: 'Include commits on or after this date (ISO 8601, e.g. "2024-01-01")',
        },
        until: {
          type: 'string',
          description: 'Include commits on or before this date (ISO 8601, e.g. "2024-12-31")',
        },
        search: {
          type: 'string',
          description: 'Filter commits whose subject line contains this string (case-insensitive)',
        },
        summary_only: {
          type: 'boolean',
          description:
            'Return only the summary (total commits, branch, contributors) without loading the full commit list',
          default: false,
        },
      },
      required: [],
