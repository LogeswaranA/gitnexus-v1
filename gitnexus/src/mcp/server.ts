 ...
 *
 * Tools: list_repos, query, cypher, context, impact, detect_changes, rename, git_log, git_show
...
function getNextStepHint(toolName: string, args: Record<string, any> | undefined): string {
  ...
  switch (toolName) {
    case 'list_repos':
      return `\n\n---\n**Next:** READ gitnexus://repo/{name}/context for any repo above to get its overview and check staleness. Use git_log({summary_only: true}) to see the latest commit, branch, and contributor stats.`;
    ...
    case 'git_log':
      return `\n\n---\n**Next:** Use git_show({commit: "<hash>"${repoParam}}) to see what changed in a specific commit, or query({query: "<topic>"${repoParam}}) to explore the related code.`;

    case 'git_show':
      return `\n\n---\n**Next:** Use query({query: "<filename or symbol>"${repoParam}}) to explore the code context of changed files, or git_log(${repoParam ? `{repo: "${repo}"}` : '{}'}) to see more commits.`;
