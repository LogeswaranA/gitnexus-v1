...
export class LocalBackend {
  ...
  /**
   * git_log tool — return captured commit history for a repo.
   *
   * Reads from .gitnexus/git-log.json (written by `gitnexus analyze`).
   * Supports filtering by author, date range, and message keyword, plus
   * limit/offset pagination for large histories.
   */
  private async gitLog(
    ...
    // Always return the summary from meta (already in memory via RepoHandle)
    const summary = (repo as any).meta?.gitLog ?? null;

    if (params.summary_only) {
      if (!summary) {
        return {
          error:
            'No git history found. Run `gitnexus analyze` to capture commit history.',
        };
      }
      return { summary };
    }

    const allCommits = await loadGitLog(storagePath);
    if (!allCommits || allCommits.length === 0) {
      return {
        error: 'No git history found. Run `gitnexus analyze` to capture commit history.',
        summary,
      };
    }
    ...
    if (params.author) {
      const needle = params.author.toLowerCase();
      commits = commits.filter(
        (c) =>
          c.author.toLowerCase().includes(needle) || c.email.toLowerCase().includes(needle),
      );
    }

    if (params.search) {
      const needle = params.search.toLowerCase();
      commits = commits.filter((c) => c.subject.toLowerCase().includes(needle));
    }

    if (params.since) {
      const sinceMs = new Date(params.since).getTime();
      if (!Number.isNaN(sinceMs)) {
        commits = commits.filter((c) => new Date(c.date).getTime() >= sinceMs);
      }
    }
    ...
    if (params.until) {
      ...
      if (!Number.isNaN(untilMs)) {
        commits = commits.filter((c) => new Date(c.date).getTime() <= untilMs);
      }
    }
  ...
  private async gitShow(
    repo: RepoHandle,
    params: { commit: string; include_diff?: boolean },
  ): Promise<any> {
    if (!params.commit?.trim()) {
      return { error: 'commit parameter is required.' };
    }
    const result = getCommitShow(repo.repoPath, params.commit.trim(), params.include_diff ?? false);
    if (!result) {
      return {
        error: `Commit "${params.commit}" not found or repo has no git history. Run \`gitnexus analyze\` first.`,
      };
    }
    return result;
  }
  ...
  async callTool(method: string, params: any): Promise<any> {
    ...
    switch (method) {
      ...
      case 'git_log':
        return this.gitLog(repo, params);
      case 'git_show':
        return this.gitShow(repo, params);
      default:
