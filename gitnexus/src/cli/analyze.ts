...
const analyzeCommandImpl = async (inputPath?: string, options?: AnalyzeOptions): Promise<void> => {
  ...
  try {
    ...
    if (result.gitLog) {
      const g = result.gitLog;
      const branchPart =
        g.currentBranch && g.currentBranch !== 'HEAD' ? ` | branch: ${g.currentBranch}` : '';
      console.log(
        `  ${g.totalCommits.toLocaleString()} commits | ${g.contributors.length} contributor${g.contributors.length !== 1 ? 's' : ''}${branchPart}`,
      );
      if (g.latestCommit) {
        console.log(`  Latest commit: ${g.latestCommit.shortHash} ${g.latestCommit.subject}`);
      }
    }
